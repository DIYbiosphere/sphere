...
## Status
The `status:` key is automatically generated by `_plugins/add-variables-to-files.rb` and we only recommend to input it manually in the _front matter_ if it should be something other than what is automatically generated. If `status:` isn't already set, the plugin marks it `active` if `start-date:` is in the past and `end-date:` isn't, `inactive` if `end-date:` is in the past, `planned` if `start-date:` is in the future, or `unknown` if there's no `start-date:` at all.
